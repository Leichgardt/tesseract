...
class TesseractBot:
    ...
    def send_notify(self, header, text):
        for chat_id in self.subs[header]:
            ...
            while True:
                try:
                    response = self.bot.send_message(chat_id=chat_id, text=text, timeout=5)
    ...
    def send_file(self, header, document):
        for chat_id in self.subs[header]:
            while True:
                response = self.bot.send_document(chat_id=chat_id, document=document, timeout=30)
                if isinstance(response, telegram.message.Message):
                    break
    ...
    def _unsubscribe(self, update, context):
        ...
        if chat_id in self.subs['all']:
            ...
            context.bot.send_message(chat_id=chat_id, text="You are not a subscriber.")
    ...
    def _set_group(self, update, context):
        ...
        if group == '':
            context.bot.send_message(chat_id=chat_id, text="Run command with /set_group NewGroup")
            return
        ...
        if group not in self.subs.keys():  # если группы нет
            ...
            context.bot.send_message(chat_id=chat_id, text=f"New group '{group}' created")
        elif chat_id not in self.subs[group]:  # если группа есть
            self.subs[group].append(chat_id)
            context.bot.send_message(chat_id=chat_id, text=f"Added in group '{group}'")
        else:  # если есть в группе
            context.bot.send_message(chat_id=chat_id, text="You are already in the group")
        self._save_subs()
    ...
    def _leave_group(self, update, context):
        ...
        if group == '':
            context.bot.send_message(chat_id=chat_id, text="Run command with /leave_group NewGroup")
            return

        if group == 'all':
            for group in self.subs.keys():
                self.subs[group].remove(chat_id)
                context.bot.send_message(chat_id=chat_id, text=f"You left all groups")
        elif group in self.subs.keys():
            self.subs[group].remove(chat_id)
            context.bot.send_message(chat_id=chat_id, text=f"You left '{group}'")
        else:
            context.bot.send_message(chat_id=chat_id, text=f"Group '{group}' doesn't exist")
